...
async function render(smiles, imgEl) {
  try {
    ...
    if (!res.ok) {
      alert("Render failed: " + res.statusText);
      return;
    }
  ...
  step.innerHTML = `
    ...
    <div class="arrowBlock">
      ...
      <svg width="140" height="40">
        <line x1="0" y1="20" x2="120" y2="20" stroke="black" stroke-width="2"/>
...
/* ---------- Delete / Undo / Redo ---------- */
function deleteSelected() { if (selectedArrow) { saveState(); selectedArrow.remove(); selectedArrow = null; } }
function undo() { if (!undoStack.length) return; redoStack.push(document.getElementById("reactionCanvas").innerHTML); restoreState(undoStack.pop()); }
function redo() { if (!redoStack.length) return; undoStack.push(document.getElementById("reactionCanvas").innerHTML); restoreState(redoStack.pop()); }
...
async function suggestMechanism() {
  ...
  if (!step) return alert("No reaction step found");
  ...
  if (!data.reactant) return alert("No reactant SMILES found");
  ...
  if (!res.ok) return alert("Suggest failed");
...
function drawSuggestedArrow(step, arrow) {
  ...
  drawArrow(svg, { x: 200, y: 140 }, { x: 350, y: 140 }); // TEMP arrow
}
